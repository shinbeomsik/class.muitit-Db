	...
	public static void open() {
		...
		b1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// 3.값들이 들어있는 가방을 전달.
				int result = dao.insert(bag); // 1 or 0 을 넘겨줌

				if (result == 1) {
					JOptionPane.showMessageDialog(f, "회원가입성공");
				} else {
					JOptionPane.showMessageDialog(f, "회원가입실패, 재입력해주세요");
				}
		...
		b2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == 1) {
					...
					JOptionPane.showMessageDialog(f, "회원탈퇴실패, 재입력해주세요");
				}
		...
		b3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == 1) {
					...
					JOptionPane.showMessageDialog(f, "회원수정실패, 재입력해주세요");
				}

			}// action
		}); // b3
		
		// 자바는 변수에 들어갈 데이터 타입 변수명을 써주어야
		// RAM 에 저장공간이 만들어진다.
		// String test = null; ==> 선언!
		// 자바는 선언시 변수가 만들어진다.
		...
		b4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				MemberDAO3 dao = new MemberDAO3();
				MemberVO bag = dao.one(id); // MemberVO 로 리턴
				if (bag != null) {
